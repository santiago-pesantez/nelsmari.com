...
(() => {
  ...
  function render() {
    ...
    // Analytics: checkout_start (solo una vez por carga de página)
    if (typeof Analytics !== 'undefined' && !window._checkoutStartTracked) {
      window._checkoutStartTracked = true;
    ...
    // Analytics: combos aplicados
    if (typeof Analytics !== 'undefined') {
      comboResult.applied.forEach(function (combo) {
        Analytics.track('combo_applied', { combo_type: combo.id });
      });
    }

    const minOrder = typeof Config !== 'undefined' ? Config.MIN_ORDER : 10.00;
    const belowMin = comboResult.total < minOrder;
    ...
    summaryEl.innerHTML = `
      ...
      ${(() => {
        ...
        if (typeof Analytics !== 'undefined') {
          tips.forEach(function (tip) {
            Analytics.track('tip_shown', { tip_text: tip.replace(/<[^>]+>/g, '') });
          });
        }
      ...
      <!-- Pedido mínimo -->
      ${belowMin ? `
        <div class="checkout-min-order">
          <svg xmlns="http://www.w3.org/2000/svg" width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><circle cx="12" cy="12" r="10"/><line x1="12" y1="8" x2="12" y2="12"/><line x1="12" y1="16" x2="12.01" y2="16"/></svg>
          <span>El pedido mínimo es de <strong>${Cart.formatPrice(minOrder)}</strong>. Agrega ${Cart.formatPrice(minOrder - comboResult.total)} más para continuar.</span>
        </div>
      ` : ''}

      <!-- Acciones -->
      <div class="checkout-actions">
        <button class="btn btn--whatsapp btn--lg btn--full ${belowMin ? 'btn--disabled' : ''}" onclick="Checkout.send()">
  ...
  window.Checkout = {
    ...
    send() {
      ...
      const minOrder = typeof Config !== 'undefined' ? Config.MIN_ORDER : 10.00;
      if (comboResult && comboResult.total < minOrder) return;
